...
var shoppingCart = (function () {
  ...
  // Constructor
  function Item(name, price, count, productId) {
    this.name = name;
    this.price = price;
    this.count = count;
  ...
  // Add to cart
  obj.addItemToCart = function (name, price, count, productId) {
    ...
    var item = new Item(name, price, count, productId);
    cart.push(item);
    saveCart();
  };
  // Set count from item
  ...
  obj.removeItemFromCart = function (productId) {
    console.log("check remove", productId);
    for (var item in cart) {
      if (cart[item].productId === productId) {
        cart[item].count--;
...
$(".add-to-cart").click(function (event) {
  ...
  shoppingCart.addItemToCart(name, price, 1, productId);
  displayCart();
});
...
function displayCart() {
  ...
  for (var i in cartArray) {
    output +=
      ...
      cartArray[i].name +
      ">+</button></div></td>" +
      // "<td><button class='delete-item btn btn-danger' data-product-id=" +
